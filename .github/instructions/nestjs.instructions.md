...
- **Guidance for Copilot:**
  - Use `@Injectable()` decorator for services and other providers
...
- Define models in `schema.prisma` with proper fields and relationships
...
- Inject `PrismaService` (extending `PrismaClient`) into services for database access; regenerate the client with `npx prisma generate` after schema changes.
- Use `select` and `include` in queries to fetch only necessary data and avoid N+1 problems (e.g., `prisma.user.findMany({ select: { id: true, username: true } })`).
- Wrap multi-step mutations in transactions using `prisma.$transaction()` to ensure atomicity (e.g., creating a message with attachments).
- Validate JSON fields like `Role.permissions` or `Message.content` with DTOs before persisting to prevent malformed data.
- Leverage `deletedAt` for soft deletes and timestamps for auditing; update enums in `schema.prisma` and regenerate the client as needed.
...
1. Use NestJS CLI for scaffolding: `npx nest generate module modules/users`
